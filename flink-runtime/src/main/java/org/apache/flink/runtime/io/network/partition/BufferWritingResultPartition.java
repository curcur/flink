...
public abstract class BufferWritingResultPartition extends ResultPartition {
	...
	public void emitRecord(ByteBuffer record, int targetSubpartition) throws IOException {
		BufferBuilder buffer = appendDataToSubpartitionForNewRecord(record, targetSubpartition);

		while (record.hasRemaining()) {
			// full buffer, partial record
			finishSubpartitionBufferBuilder(targetSubpartition);
			buffer = appendDataToSubpartitionForRecordContinuation(record, targetSubpartition);
		}
	...
	public void broadcastRecord(ByteBuffer record) throws IOException {
		BufferBuilder buffer = appendDataToBroadcastForNewRecord(record);

		while (record.hasRemaining()) {
			// full buffer, partial record
			finishBroadcastBufferBuilder();
			buffer = appendDataToBroadcastForRecordContinuation(record);
		}
