 ...
 * Tests for failover.
 */
public class ApproximateLocalRecoverySimpleISameTMTCase {
	...
	public void setup() throws Exception {
		...
		cluster = new MiniClusterWithClientResource(
			new MiniClusterResourceConfiguration.Builder()
				...
				.setNumberSlotsPerTaskManager(3).build());
		cluster.before();
	}
	...
	public void localTaskFailureRecovery() throws Exception {
		...
		source.map(new FailingMapper<>(failAfterElements)).setParallelism(1)
			.addSink(new ValidatingExactlyOnceSink(numElementsPerTask * producerParallelism, producerParallelism)).setParallelism(1);
	...
	private static class ValidatingExactlyOnceSink extends RichSinkFunction<Tuple3<Integer, Long, Integer>> implements ListCheckpointed<Tuple2<Integer, BitSet>> {
		...
		public ValidatingExactlyOnceSink(int numElementsTotal, int producerParallelism) {
			this.numElementsTotal = numElementsTotal;
			this.producerParallelism = producerParallelism;
		}

		@Override
		public void invoke(Tuple3<Integer, Long, Integer> element) throws Exception {
			numElements++;

			int value = element.f0 * producerParallelism + element.f2;
